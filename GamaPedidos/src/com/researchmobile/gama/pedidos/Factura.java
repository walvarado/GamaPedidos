...
public class Factura extends Activity implements TextWatcher, OnItemClickListener{
 ...
 final DataBase objeto = new DataBase(Factura.this);
	 
	 try {
		objeto.abrir();		 
		 Cursor c = objeto.consultaSumaAbonos(getCliente().getCliCodigo());
		 int tamano = c.getCount();
		 EstadoDeCuenta estados[] =  new EstadoDeCuenta[tamano];
		 
		 int contador  = 0;
		 for(c.moveToFirst(); !c.isAfterLast(); c.moveToNext())
		{		EstadoDeCuenta estadosTemp = new EstadoDeCuenta();
				
				estadosTemp.setCodigocliente(c.getString(0));
				estadosTemp.setMovdocumento(c.getString(1));
				estadosTemp.setFecha(c.getString(2));
				estadosTemp.setTotal(c.getString(3));
				estadosTemp.setCredito(c.getString(4));
				estadosTemp.setPagado(c.getString(5));
				estadosTemp.setSaldo(c.getString(6));
				estadosTemp.setCancelado(Integer.parseInt(c.getString(7)));
				estadosTemp.setSinc(Integer.parseInt(c.getString(8)));
		
				estados[contador] = estadosTemp;
				contador++;
		}
		 setCuentas(estados);
	 ...
	 setListaEstadosDeCuentaTest(new ListaTest());
		...
		setSimpleAdapter( 
	         	new SimpleAdapter(this, 
	         					  ...
	         					  getListaEstadosDeCuentaTest().ListaEstadosDeCuenta(getCuentas()),	
...
private void Guardar() {
	...
	conexionCliente.GuardaEstadoDeCuenta(getCliente().getEstadoCuentas(), Factura.this); // Guarda en la base de datos los ESTADOS DE CUENTA nuevos
...
private void VerDialog(final String saldo, final String movDocumento) {
	...
	LayoutInflater factory = LayoutInflater.from(Factura.this);            
